...
async function generator ({ config, order, src, stylelintrc }: { config: any, order: any, src: string, stylelintrc: string }) {
	...
	;[...propertyOrder, "others"]
		.forEach((property) => {
			css[property] = css[property]
				...
				.map((current) => ((!current.startsWith("-{0,1}") && current.includes(String.raw`\d`) && !current.includes("gray") && !current.includes("black"))
					? `-{0,1}${ current }`
					: current)) // Add optional negative `-{0,1}` for classes with numbers that are not colours
				.filter((current, index, array) => !(!current.startsWith("-{0,1}") && array.includes(`-{0,1}${ current }`))) // Remove redundant classes after adding optional negative `-{0,1}`
		})
