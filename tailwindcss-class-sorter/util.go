...
func normaliseConfig(input []byte, toOutputErr bool) TConfigInput {
	var (
	...
	strArr = append(groupStarArr, configInput.VariantOrdering[index+1:]...)
	configInput.VariantOrdering = append(configInput.VariantOrdering[:index], strArr...)

	/* ---expand peer-*--- */
	index = slices.Index(configInput.VariantOrdering, "peer-*")
	strArr = append(peerStarArr, configInput.VariantOrdering[index+1:]...)
	configInput.VariantOrdering = append(configInput.VariantOrdering[:index], strArr...)
...
func normaliseOrder(order TOrderList, editOrder TEditOrder) TOrderList {
	...
	for _, v := range editOrder.Append {
		...
		switch {
		case appendIndex < 0:
			order = append(TOrderList{TOrderGroup{v.GroupName, v.RegexList}}, order...)

		default:
			order = append(order[:appendIndex], append(TOrderList{TOrderGroup{v.GroupName, v.RegexList}}, order[appendIndex:]...)...)
		}
	}
