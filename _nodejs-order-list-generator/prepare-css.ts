...
/*
const colorStr = Object.keys(tailwindColours)
...
await (async function () {
	...
	const css: string = await readFile(getArgv("--src"), { encoding: "utf8", flag: "r" })
		...
		.then((string_: string) => string_
			...
			.replaceAll(/ > [^{]+{/g, " {") // Remove siblings selector
			.replaceAll(/(\.[\dA-Za-z-]+?):[^,]+?, ?\./g, "$1, .") // Remove pseudo-class
			.replaceAll(/(\.[\dA-Za-z-]+?):[^{]+? ?{/g, "$1 {"), // Remove pseudo-class
		)
		/* */
		.then((string_: string) => string_
		...
		.then((array: string[]) => array.reduce((accumulator: string[], line: string) => {
			if (line.search(/\.\w[\w./\\-]+, \.\w[\w./\\-]+/) > -1) {
				...
				line = classes.reduce(
					(stringAccumulator, className) => `${ stringAccumulator }${ stringAccumulator.length > 0 ? "\n" : "" }${ className } ${ styles }`,
					"",
				)
			}
		...
		/* */
		.then((array: string[]) => [...new Set(array)]) // Remove duplicate classes and styles
		.then((array: string[]) => array.join("\n"))
		.catch((error: Error) => { console.log(`${ error.name }: ${ error.message }`) })
		?? ""

	await writeFile(getArgv("--src"), `${ css }\n`, { encoding: "utf8", flag: "w" })
