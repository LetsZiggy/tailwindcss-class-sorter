...
func printHelp(
	...
	helpStrings["format"]["code"] = []string{
		...
		"- if set, \"--code-ext\" flag must also be set",
	...
	helpStrings["format"]["codeExt"] = []string{
		...
		"- if \"--code\" flag is set, \"--code-ext\" flag must also be set",
	}
	...
	helpStrings["list"]["showEditedOrder"] = []string{
		...
		"- don't use edited order-list as reference for \"edit_order\" config; option instead, use the unedited order-list as reference by removing \"--show-edited-order\" flag",
	}
...
func helpCommand(fullHelp bool) {
	/* ---main command--- */
	fmt.Print(
		padString([]string{"tailwindcss-class-sorter [subcommands] [options?] [globs?]"}, "", 0, 0, 0, 2, false, ""),
	...
	fmt.Print(
		...
		padString(linebreak("Prints out order-list in prettified json string or in base64-encoded string. The \"--show-edited-order\" flag will output the order-list with \"edit_order\" config option implemented. Use the unedited order-list as reference for \"edit_order\" config option", 75), "  ", 2, 0, 0, 1, false, ""),
	)
	...
	if fullHelp {
		fmt.Print(
			...
			padString([]string{"--show-edited-order"}, "  ", 3, 0, 0, 1, false, ""),
