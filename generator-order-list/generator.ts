/* eslint-disable unicorn/no-break-in-nested-loop, unicorn/no-negated-array-predicate, unicorn/no-unreadable-for-of-expression */
...
const endSortOrder: NonEmptyArray<string> = [
	...
	"-x", "-y",
	"-px", "-lh", "-fr",
	"-cqb", "-cqi", "-cqmin", "-cqmax", "-cqw", "-cqh",
	"-dvb", "-dvi", "-dvmin", "-dvmax", "-dvw", "-dvh",
	"-svb", "-svi", "-svmin", "-svmax", "-svw", "-svh",
	"-lvb", "-lvi", "-lvmin", "-lvmax", "-lvw", "-lvh",
	"-vb", "-vi", "-vmin", "-vmax", "-vw", "-vh",
...
export async function generator (configOrder: StylelintConfigOrder, source: string, stylelintrcPath: string): Promise<Record<"order", OrderData[]>> {
	...
	// Add additional css properties to order (see prepare-css.ts)
	{
		const before = 0
		const after = 1
		let configOrderKeys: string[] = Object.keys(configOrder.order)
		let index
		...
		index = configOrderKeys.indexOf("flex")
		configOrderKeys = [...configOrderKeys.slice(0, index + after), "flex-#>", ...configOrderKeys.slice(index + after)]

		// ### SPACING ###

		// --- MARGIN ---
		// `<#-margin` property to be added ---before--- `margin` property in generator.ts (see prepare-css.ts)
		index = configOrderKeys.indexOf("margin")
		configOrderKeys = [...configOrderKeys.slice(0, index + before), "<#-margin", ...configOrderKeys.slice(index + before)]

		// ### SIZING ###

		// --- WIDTH | HEIGHT ---
		// `<#-width` property to be added ---before--- `width` property in generator.ts (see prepare-css.ts)
		index = configOrderKeys.indexOf("width")
		configOrderKeys = [...configOrderKeys.slice(0, index + before), "<#-width", ...configOrderKeys.slice(index + before)]

		// ### BORDERS ###

		// --- BORDER-WIDTH | BORDER-COLOR | BORDER-STYLE ---
		// `<#-border` property to be added ---before--- `border` property in generator.ts (see prepare-css.ts)
		index = configOrderKeys.indexOf("border")
		configOrderKeys = [...configOrderKeys.slice(0, index + before), "<#-border", ...configOrderKeys.slice(index + before)]
		...
		index = configOrderKeys.indexOf("border-style")
		configOrderKeys = [...configOrderKeys.slice(0, index + before), "<#-border-style", ...configOrderKeys.slice(index + before)]

		configOrder.order = configOrderKeys
			.reduce((accumulator, current) => {
				accumulator[current] = []

				return accumulator
			}, {} as StylelintConfigOrder["order"])
	}

	// Get classnames from css
	{
		const configOrderKeys: string[] = Object.keys(configOrder.order)
		let sourceData: string | string[] | undefined

		try { sourceData = await readFile(source, { encoding: "utf8", flag: "r" }) }
		catch (error) { throw new Error("invalid filepath: --src", { cause: error }) }

		sourceData = sourceData.split("\n")

		if (configOrderKeys.length === 0) {
			throw new Error(`"order" is empty`)
		}

		const cssProperties: NonEmptyArray<string> = [...configOrderKeys, "others"] as unknown as NonEmptyArray<string>

		// Setup css sorting
		const css: Prettify<SortedClassnames> = cssProperties
			.reduce((accumulator, current) => {
				accumulator[current] = []
		...
		for (const data of sourceData) {
			...
			for (const property of configOrderKeys) {
					...
					if (css[property].length > 0 && !foundRelatedProperties.has(property)) {
						switch (true) {
							...
							case (
								...
								relatedProperties = configOrderKeys.filter((current) => {
									return (
		...
		for (const property of cssProperties) {
			...
			// Sort classnames according to alphabetical groupname order and prefix
			{
				const classnamesParts = css[property].map((current) => {
					...
					const groupname = withoutPrefix.split("-", 1).at(0)!
		...
		/* eslint-disable @stylistic/function-paren-newline */
		classnames = [...new Set(
			Object.values(css).flat(Infinity) as string[],
		...
		/*
		{
			...
			for (const property of cssProperties) {
				...
				const propertyWithMultipleGroupnames = new Set<string>(css[property].map((current) => getClassnameParts(current).withoutPrefix.split("-", 1).at(0)!))
			...
			// 	assert.ok(Array.isArray(css[current]))

			// 	accumulator[current] = css[current]
			// 		.map((classname) => classname
			// 			.replaceAll("-(?:(?:inherit)|(?:current)|(?:transparent)|(?:black)|(?:white))", "-(?:(?:white))")
			// 			.replaceAll("-(?:(?:red)|(?:orange)|(?:amber)|(?:yellow)|(?:lime)|(?:green)|(?:emerald)|(?:teal)|(?:cyan)|(?:sky)|(?:blue)|(?:indigo)|(?:violet)|(?:purple)|(?:fuchsia)|(?:pink)|(?:rose)|(?:slate)|(?:gray)|(?:zinc)|(?:neutral)|(?:stone)|(?:taupe)|(?:mauve)|(?:mist)|(?:olive))-", "-(?:(?:red))-"))
		...
		/*
		{
			const cssjson = JSON.stringify(css, undefined, "")
				.replaceAll("-(?:(?:inherit)|(?:current)|(?:transparent)|(?:black)|(?:white))", "-(?:(?:white))")
				.replaceAll("-(?:(?:red)|(?:orange)|(?:amber)|(?:yellow)|(?:lime)|(?:green)|(?:emerald)|(?:teal)|(?:cyan)|(?:sky)|(?:blue)|(?:indigo)|(?:violet)|(?:purple)|(?:fuchsia)|(?:pink)|(?:rose)|(?:slate)|(?:gray)|(?:zinc)|(?:neutral)|(?:stone)|(?:taupe)|(?:mauve)|(?:mist)|(?:olive))-", "-(?:(?:red))-")

			console.log(cssjson)

			// Reduce `order_list.json` size for debugging
			classnames = classnames
				.map((current) => current
					.replaceAll("-(?:(?:inherit)|(?:current)|(?:transparent)|(?:black)|(?:white))", "-(?:(?:white))")
					.replaceAll("-(?:(?:red)|(?:orange)|(?:amber)|(?:yellow)|(?:lime)|(?:green)|(?:emerald)|(?:teal)|(?:cyan)|(?:sky)|(?:blue)|(?:indigo)|(?:violet)|(?:purple)|(?:fuchsia)|(?:pink)|(?:rose)|(?:slate)|(?:gray)|(?:zinc)|(?:neutral)|(?:stone)|(?:taupe)|(?:mauve)|(?:mist)|(?:olive))-", "-(?:(?:red))-")) as NonEmptyArray<string>
		}
		/* */
	}
	...
	for (const groupnameSortOrder of groupnameSortOrders) {
		const groupnames = classnames.map<string>((current) => getClassnameParts(current).withoutPrefix.split("-", 1).at(0)!)
	...
	for (const classname of classnames) {
		const groupname: string = getClassnameParts(classname).withoutPrefix.split("-", 1).at(0)!
