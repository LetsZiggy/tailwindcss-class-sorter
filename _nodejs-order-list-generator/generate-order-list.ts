...
function getRepeatedProperty (data: OrderData[]) {
...
function joinUpArrays (data: OrderData[], propertyName: string, { end = 999, excludes = [], mergeTo = 0 }: JoinUpArraysOptions = {}) {
	...
	const groupMerged: OrderData = indexList
		...
		.reduce((accumulator: OrderData, current) => {
...
function reorderStyles (data: OrderData[], order: string[]) {
	...
	const groupsArray: OrderData[] = indexList
		.reduce((accumulator: OrderData[], current: number) => [...accumulator, data[current]!], [])
	...
	data.splice(indexList.toSorted((a: number, b: number) => a - b).at(0)!, 0, ...groupsArray)
