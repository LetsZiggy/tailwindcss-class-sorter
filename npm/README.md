...
```bash
npm install --save-dev github:LetsZiggy/tailwindcss-class-sorter/npm

pnpm add --save-dev github:LetsZiggy/tailwindcss-class-sorter/npm
```
<!-- NPM::END -->
...
Subcommands:
  format [options] [globs?]
    Formats files through multiple space separated paths/globs, or piping in
    base64-encoded string with the "--stdin" flag, or streaming in base64-encoded
    heredoc/herestring with the "--stdin" flag
    ...
    Options:
      ...
      --stdin
          - by piping in base64-encoded string
          	(eg. base64 -w 0 example.html | twcs format --stdin --ext html)
          - by streaming in base64-encoded heredoc/herestring
          	(eg:herestring. twcs format --stdin --ext html <<< $(base64 -w 0 example.html))
      ...
      --ext [string]
          - indicates the filetype of the "--stdin" flag base64-encoded string
  ...
  list [options]
    ...
    Options:
      --config [string]
          - see https://github.com/LetsZiggy/tailwindcss-class-sorter/blob/main/dist/config.json
          	for full list of config options
          - "--config" flag is optional; partial config is valid; unset config options will
          	fallback to the default config values
          - if "--base64-config" flag is set, "--config" flag expects base64-encoded json string
          - if "--base64-config" flag is not set, "--config" flag expects a valid path to
          	json config file
      --base64-config
          - if set, "--config" flag expects base64-encoded json string
          - if not set, "--config" flag expects a valid path to json config file
...
- Where is `v1.*.*`, `v2.*.*`, and `v3.*.*`?
	- Releases will follow tailwindcss major versions and minor versions from v4.0.0 onwards.
