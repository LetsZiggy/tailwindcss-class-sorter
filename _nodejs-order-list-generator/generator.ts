...
function sorting (a: string, b: string): (-1 | 0 | 1) {
	const lastCount = Math.min(a.length, b.length) - 1
	let count = 0
	let sort: -1 | 0 | 1 = 0
...
export async function generator ({ config, order, source, stylelintrcPath }: GeneratorArguments): Promise<Record<"order", OrderData[]>> {
	...
	const css = sourceData.reduce((accumulator: { others: string[], [key: string]: string[] }, current: string) => {
		let isFound = false
	...
	const screenList = ["xs", ...Object.keys(tailwindConfig.theme.screens)]
		.filter((current: string) => current.search(/\dxl/g) === -1)
	...
	const list = flattened
		.reduce((accumulator: Array<{ group_name: string, regex: string[] }>, current: string) => {
